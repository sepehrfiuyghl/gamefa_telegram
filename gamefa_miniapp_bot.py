...
async def miniapp_command(message: Message):
    if not is_admin(message):
        return
    if not MINI_APP_URL:
        await message.answer("⚠️ MINI_APP_URL در Railway تنظیم نشده است.")
        return
    keyboard = InlineKeyboardMarkup(inline_keyboard=[[
        InlineKeyboardButton(text="🖥️ باز کردن پنل Gamefa", web_app=WebAppInfo(url=MINI_APP_URL))
    ]])
    await message.answer(
        "✨ <b>پنل تحریریه Gamefa</b>\n\n"
        "نسخه Mini App آماده است. از دکمه زیر وارد پنل شوید.",
        parse_mode=ParseMode.HTML,
